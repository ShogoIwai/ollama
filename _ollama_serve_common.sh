...
# NOTE: keep this in sync with CLAUDE_CODE_MAX_CONTEXT_TOKENS in source_local.sh /
...
# ~1.5x and may spill to CPU on a 24 GB GPU (slower). 128K grows it further, and
# 256K (262144) — the qwen35moe family's native window — spills ~6% to CPU on a
# 24 GB GPU (~25 GB, ~89 tok/s vs ~110 at 128K/100% GPU). We default to 256K so
# every local model exposes its full window; if `ollama ps` shows a large CPU
# split or OOM, drop OLLAMA_KV_CACHE_TYPE to q4_0 or lower this value (e.g.
# OLLAMA_CONTEXT_LENGTH=128000 ./start_ollama_<name>.sh for 100% GPU).
